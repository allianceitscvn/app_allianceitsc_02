...
if (
  ...
  PushManager.supportedContentEncodings =
    parseInt(RegExp.$1) >= 55 ? ["aes128gcm", "aesgcm"] : ["aesgcm"];
}
...
function requestPushSubscription(registration) {
  ...
  if (authType === "vapid") opt.applicationServerKey = serverKey;
  return registration.pushManager
    .subscribe(opt)
    .then(getSubscription, errorSubscription);
}
  ...
  if (subscription) {
    ...
    if ("getKey" in subscription) {
      ...
      try {
        ...
        const useAesgcm = navigator.userAgent.match(/Firefox\/(\d+)/)
          ? parseInt(RegExp.$1) >= 46
            ? 1
            : 0
        ...
        const idx =
          encodings instanceof Array ? encodings.indexOf("aes128gcm") : -1;
        arg.contentEncoding =
          idx >= 0 ? "aes128gcm" : useAesgcm ? "aesgcm" : "aesgcm128";
      } catch (e) {}
    }
    if (authType === "vapid") {
      arg.jwt = {
...
function init() {
  if ("serviceWorker" in navigator) {
    ...
    if (authType) {
      ...
      setAuthType({
        currentTarget: { value: _("authvapid").checked ? "vapid" : "gcm" },
      });
